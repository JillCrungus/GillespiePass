...
LINK_ENTITY_TO_CLASS(npc_portal_turret, CNPC_PortalFloorTurret);
LINK_ENTITY_TO_CLASS(npc_portal_turret_floor, CNPC_PortalFloorTurret);
...
void CNPC_PortalFloorTurret::Activate(void)
{
	CNPC_FloorTurret::Activate();
	m_iLastState = TURRET_AUTO_SEARCHING;
}

void CNPC_PortalFloorTurret::Spawn(void)
{
	CNPC_FloorTurret::Spawn();
	SetModel("models/props/turret_01.mdl");
	LaserOn();
}

void CNPC_PortalFloorTurret::Precache(void)
{
...
int CNPC_PortalFloorTurret::OnTakeDamage(const CTakeDamageInfo &info)
{
	...
	return CNPC_FloorTurret::OnTakeDamage(info);
}

bool CNPC_PortalFloorTurret::PreThink(turretState_e state)
{
	...
	if (iNewState != TURRET_TIPPED && iNewState < TURRET_STATE_TOTAL)
		return CNPC_FloorTurret::PreThink((turretState_e)iNewState);

	return true;
}

void CNPC_PortalFloorTurret::LaserOff(void)
{
	m_bLaserOn = false;
}

void CNPC_PortalFloorTurret::LaserOn(void)
{
	m_bLaserOn = true;
}

/*
void CNPC_PortalFloorTurret::RopesOn(void)
{
...
void CNPC_PortalFloorTurret::RopesOff(void)
{
	for (int iRope = 0; iRope < PORTAL_FLOOR_TURRET_NUM_ROPES; ++iRope)
	{
...
void CNPC_PortalFloorTurret::StartTouch(CBaseEntity *pOther)
{
	CNPC_FloorTurret::StartTouch(pOther);
	...
	if (m_pMotionController->Enabled())
	{
		...
		if (!pOther->IsPlayer() && pOther->GetMoveType() == MOVETYPE_VPHYSICS && !(pTurretPhys && ((pTurretPhys->GetGameFlags() & FVPHYSICS_PLAYER_HELD) != 0)))
		{
			...
			CNPC_PortalFloorTurret *pPortalFloor = dynamic_cast<CNPC_PortalFloorTurret*>(pOther);
			...
			if (pPortalFloor && m_bEnabled && m_bLaserOn)
			{
...
void CNPC_PortalFloorTurret::OnPhysGunPickup(CBasePlayer *pPhysGunUser, PhysGunPickup_t reason)
{
	if (m_lifeState == LIFE_ALIVE && m_bEnabled)
	{
		m_bActive = true;
		SetThink(&CNPC_PortalFloorTurret::HeldThink);
	}

	CNPC_FloorTurret::OnPhysGunPickup(pPhysGunUser, reason);
}

Vector CNPC_PortalFloorTurret::GetAttackSpread(CBaseCombatWeapon *pWeapon, CBaseEntity *pTarget)
{
...
void CNPC_PortalFloorTurret::HeldThink(void)
{
...
void CNPC_PortalFloorTurret::TippedThink(void)
{
...
Class_T CNPC_PortalFloorTurret::Classify()
{

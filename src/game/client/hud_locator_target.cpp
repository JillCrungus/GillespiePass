...
#include "gp_instructor_helpers.h"
...
ConVar locator_fade_time( "locator_fade_time", "0.5", FCVAR_NONE, "Number of seconds it takes for a lesson to fully fade in/out." );
ConVar locator_lerp_speed( "locator_lerp_speed", "5.0f", FCVAR_NONE, "Speed that static lessons move along the Y axis." );
ConVar locator_lerp_rest( "locator_lerp_rest", "1.25f", FCVAR_NONE, "Number of seconds before moving from the center." );
ConVar locator_lerp_time( "locator_lerp_time", "1.0f", FCVAR_NONE, "Number of seconds to lerp before reaching final destination" );
...
ConVar locator_text_glow_color( "locator_text_glow_color", "0 0 0 255", FCVAR_NONE, "Color of text glow" );
...
ConVar locator_staticpos_center( "locator_staticpos_center", "1", FCVAR_NONE, "If enabled, use coordinates as center point" );
ConVar locator_staticpos_x( "locator_staticpos_x", "0.5f", FCVAR_NONE );
ConVar locator_staticpos_y( "locator_staticpos_y", "0.75f", FCVAR_NONE );
...
void CLocatorTarget::UpdateVguiTarget( void )
{
	...
	ClientModeHLNormal *m_hlClientMode = dynamic_cast<ClientModeHLNormal *>(GetClientModeNormal());
	if (NULL != m_hlClientMode)
	{
		m_hVguiTarget = m_hlClientMode->GetPanelFromViewport(pchVguiTargetName);
	}
...
void CLocatorTarget::SetBinding( const char *pszBinding )	
{
	...
	while ( pchToken )
	{
		...
		const char *pchBinding = engine->Key_LookupBinding( szToken );
		...
		while ( m_iBindingChoicesCount < MAX_LOCATOR_BINDINGS_SHOWN && pchBinding )
		{
			...
			pchBinding = engine->Key_LookupBinding( szToken );
		}
...
private:
	...
	CPanelAnimationVar( int, m_iShouldWrapStaticLocators, "WrapStaticLocators", "1" );
	...
	int				m_staticIconPositionX;// Helps us stack static icons
	int				m_staticIconPositionY;// Helps us stack static icons
...
{
	//ASSERT_LOCAL_PLAYER_RESOLVABLE(); // JB Disabled, this seems to fail on non-splitscreen games (ie us)
...
int Locator_AddTarget()
{
	if( s_pLocatorPanel == NULL )
	{
		...
		ClientModeHLNormal *m_hlClientMode = dynamic_cast<ClientModeHLNormal *>(GetClientModeNormal());
		CLocatorPanel *pLocator = new CLocatorPanel(m_hlClientMode->GetViewport(), "LocatorPanel");

		//CLocatorPanel *pLocator = new CLocatorPanel( GetClientModeJBNormal()->GetViewport(), "LocatorPanel" );
...
void CLocatorPanel::Paint( void )
{
	...
	m_staticIconPositionX = ScreenWidth() * locator_staticpos_x.GetFloat();
	m_staticIconPositionY = ScreenHeight() * locator_staticpos_y.GetFloat();
...
/*
	...
	int bgcolor[4];
	sscanf( locator_background_color.GetString(), "%d %d %d %d", &(bgcolor[0]), &(bgcolor[1]), &(bgcolor[2]), &(bgcolor[3]) );
	bgcolor[ 3 ] *= fAlpha;

	int bordercolor[4];
	sscanf( locator_background_border_color.GetString(), "%d %d %d %d", &(bordercolor[0]), &(bordercolor[1]), &(bordercolor[2]), &(bordercolor[3]) );
	bordercolor[ 3 ] *= fAlpha;
...
{
	//if ( !locator_staticpos_center.GetBool() )

	int centerX = m_staticIconPositionX;
	int centerY = m_staticIconPositionY;
	...
	pTarget->m_iconX = centerX - ( totalWide * ( locator_staticpos_center.GetBool() ? 0.5f : 0.0f ) );
...
void CLocatorPanel::DrawTargetCaption( CLocatorTarget *pTarget, int x, int y, bool bDrawMultiline )
{
	...
	if ( locator_text_glow.GetBool() )
	{
		...
		int color[4];
		sscanf( locator_text_glow_color.GetString(), "%d %d %d %d", &(color[0]), &(color[1]), &(color[2]), &(color[3]) );

		vgui::surface()->DrawSetTextColor( color[0], color[1], color[2], ( color[3] / 255.0f ) * pTarget->m_alpha );
